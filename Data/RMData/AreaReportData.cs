...
namespace Data.RMData
{
    ...
    {
        public RM_AreaReport GetEntity(DataProvider dp ,Guid? id)
        {
            if (!id.HasValue)
            {
                return null;
            ...
            {
                return dp.RM_AreaReport.FirstOrDefault(m => m.Id == id);
            }
        }


        public List<AreaReportModel> GetReportList(DataProvider dp, AreaReportFilter filter, out int total,bool IsPage = true)
        {
            string waitsubmit = Model.EnumModel.ReportStatus.WaitSubmit.ToString();
            var list = from hr in dp.RM_AreaReport.Where(m => !m.IsDel&&m.ReportStatus!= waitsubmit)
