...
namespace Data.RMData
{
    ...
    {
        public RM_AssetsReport GetEntity(DataProvider dp ,Guid? id)
        {
            if (!id.HasValue)
            {
                return null;
            ...
            {
                return dp.RM_AssetsReport.FirstOrDefault(m => m.Id == id);
            }
        }


        public List<AssetsReportModel> GetReportList(DataProvider dp, AssetsReportFilter filter, out int total,bool IsPage = true)
        {
            string waitsubmit = Model.EnumModel.ReportStatus.WaitSubmit.ToString();
            var list = from hr in dp.RM_AssetsReport.Where(m => !m.IsDel&&m.ReportStatus!= waitsubmit)
            ...
            {
                list = list.Where(m => filter.SignAppraiserText.Contains(m.SignAppraiserText));
            }
